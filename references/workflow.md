...
| 执行编排 | 是否需要多 agent / sub-agent 协同，哪些任务可以并行 |
...
复杂评测可以设计多 agent / sub-agent 协同，但只有当用户明确要求或当前环境允许时才实际启用。协同的目标是缩短等待时间，并让不同任务互不干扰。

适合并行的任务：

| 任务 | 适合交给谁 | 产出 |
| --- | --- | --- |
| 代码和文档阅读 | explorer / 只读子任务 | 职责边界、入口、依赖和风险清单 |
| 数据准备 | worker / 数据子任务 | 样本池、清洗脚本、拆分结果 |
| 测试执行 | worker / 执行子任务 | 运行日志、结果文件、错误记录 |
| 评判器实现 | worker / 评判子任务 | 代码评判、规则评判、模型评判配置 |
| 人工复核整理 | 主 agent 或人工 | 分歧样本表、复核口径 |
