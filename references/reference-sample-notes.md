# 参考样例笔记

这份笔记提炼 `wechat_source + content_scoring` 评估报告中可复用的做法。它不是固定模板，使用时仍应优先遵循工作法正文。

## 样例中值得复用的做法

- 将两个相互依赖的 skill 作为一条组合链路评测。
- 固定使用三层结构：
  - `运行框架层`：API key、微信 API、日志、输出目录、成本、耗时。
  - `能力链路层`：文章获取、回补、全文详情、清洗、评分、排序。
  - `业务效果层`：是否识别出有价值的 AI 新闻、误判、漏判、边界样本。
- 使用真实执行证据：
  - 四个阶段均成功运行。
  - 全文详情使相关内容识别从 1/3 提升到 2/3。
  - 46 条原始候选去重后形成 44 条样本。
  - 16 条分歧样本沉淀为待复核资产。
- 不做加权总分。
- 将发现的问题转化为具体的 skill 优化方向。

## 已补齐的方法论要求

更新后的样例还补充了：

- `可唤起性`：检查 skill name / description、目标提示词、相似 skill 干扰、负例提示词。
- `存在必要性`：与无 skill 基线对比；未实际运行时，先尝试独立会话或隔离 prompt 基线，客观无法运行时才标为 `待补测`。
- 按阶段选择数据集拆分：该链路被视为 `成长期`，因此优化集占主要比例，同时保留一部分样本用于阶段性验收。
- 评判方式选择规则：代码、规则、模型、人工分别用于不同问题类型。
- 模型评判输出结构：对齐 key / score / comment 这类可聚合、可解释的格式。

## 可复用报告顺序

使用这个顺序：

1. 执行摘要
2. Skill 理解与分层
3. 评测目标与指标体系
4. 评测设计确认
5. 评测流程设计
6. 测试方法与执行流程
7. 测试结果
8. Skill 优化方向
9. 产物与复现位置
10. 出处与参考

## 使用提醒

如果报告在测试完成后追加了新的方法要求，不要倒推或编造结果。应先主动补做能用当前工具完成的检查；确实无法补做的项目，再新增清晰说明并标记为 `待补测` 或 `间接证据`，同时写明已尝试路径和阻塞原因。
