...
它会根据目标 `SKILL.md`、本地仓库、代码、日志、trace、metrics、测试产物和参考工作法，完成场景与对象理解、三层拆解、指标设计、测试执行或复盘、失败归因，并输出一份结构化、可复用的评估报告。

正式执行前，它会先产出评测设计草案，包括三层拆解、目标/指标、数据方案、评判方式、数据记录表和测试流程；确认后再进入测试执行和最终报告生成。
...
- 测试不能污染生产环境数据；有写入、发送、发布或删除风险时，先使用沙箱、模拟环境或替身接口。
- 不默认加权汇总总分，除非用户明确要求。
- 在线文档更新优先局部修改，尽量保留评论。
