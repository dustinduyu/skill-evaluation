...
- 分层、评测目标、指标、数据方案和执行编排形成草案后，必须先给用户确认；用户确认前不要进入大规模测试、在线写入或最终报告生成。
- 必须包含两个 Skill 专项目标：
...
5. **确认评测设计**
   - 输出一版评测设计草案，至少包含：三层拆解、评测目标、指标表、数据来源、数据集拆分、评判方式、数据记录表结构和执行编排。
...
6. **设计测试流程和协同编排**
   ...
   - 判断是否需要多 agent / sub-agent 协作：当任务可拆成互不冲突的代码阅读、数据准备、测试执行、结果复核时，可以设计协同编排；只有在用户明确要求或当前环境允许时才实际启用。
